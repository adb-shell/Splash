...
class BottomTabDataSource(private val bottomTabRepository: BottomTabRepository,
                          private val intialpaginationData: BottomTabPaginationData): PageKeyedDataSource<BottomTabPaginationData,Photos>() {

    val networkState:MutableLiveData<PhotoFeedNetworkState> = MutableLiveData()

    override fun loadBefore(params: LoadParams<BottomTabPaginationData>, callback: LoadCallback<BottomTabPaginationData,Photos>) {}

    override fun loadInitial(params: LoadInitialParams<BottomTabPaginationData>, callback: LoadInitialCallback<BottomTabPaginationData,Photos>){
        bottomTabRepository.getFeeds(pageno = 1,type = intialpaginationData.mode,successhander = { photos->
    ...
    override fun loadAfter(params: LoadParams<BottomTabPaginationData>, callback: LoadCallback<BottomTabPaginationData,Photos>) {
        networkState.postValue(PhotoFeedNetworkState.FeedNetworkPaginationLoading)
        bottomTabRepository.getFeeds(pageno = params.key.pagenumber,type = params.key.mode,successhander = {photos->
