...
class BottomTabRepository(
    ...
    fun getFeeds(
        ...
        when {
            internetHandler.isInternetAvailable() -> getFreshFeeds(pageno,
                    type,
                    successhander,
                    errorhandler)
            cache.isCacheAvail()                  -> getFeedsFromLocalDb(pageno,
                    type,
                    successhander,
                    errorhandler)
    ...
    private fun getFeedsFromLocalDb(
            pageno: Int = 1,
            type: String,
            successhander: (ArrayList<Photos>) -> Unit,
            errorhandler: (e: Throwable) -> Unit
    ) {
        disposable.add(localdb.getPhotos(pageno, type)
                .subscribeWith(object : DisposableMaybeObserver<PhotosStorage>() {
                    override fun onSuccess(storagephotos: PhotosStorage) =
                            successhander(storagephotos.photos)
